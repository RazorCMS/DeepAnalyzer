#include "InclusiveControlRegion.h"
...
void InclusiveControlRegion::Analyze(bool isData, int option, string outputfilename, string label)
{
    ...
    if (outfilename == "") outfilename = "InclusiveControlRegion.root";
    ...
    for (Long64_t jentry=0; jentry<fChain->GetEntries();jentry++) 
    {
        ...
        if (treeTypeOption == 5 || treeTypeOption == 15) 
        {
            for (int i = 0; i < nPhotons; i++)
            {
                ...
                //NOTE not running these for MC because trigger is broken in 80X samples. Update: Trigger is no longer broken since MC_Summer16
                //if (isData) 
                {
        ...
        if (GoodPhotons.size()>0) 
        {
            ...
            {
                if (GoodJetsNoLeadPhoton[subtractedIndex].Pt() < JET_PT_CUT) 
                { //erase this jet
        ...
        for (auto& lep : GoodLeptons)
        {
            ...
            if(subtractedIndex >= 0) 
            {
                if(GoodJetsNoLeptons[subtractedIndex].Pt() < JET_PT_CUT)
                { //erase this jet
