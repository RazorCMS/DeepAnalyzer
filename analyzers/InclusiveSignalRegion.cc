#include "InclusiveSignalRegion.h"
...
void InclusiveSignalRegion::Analyze(bool isData, int option, string outFileName, string label)
{
    /////////////////////////////////
    //Basic setup
    /////////////////////////////////
    ...
    {
        cout << "InclusiveSignalRegion: Output filename not specified!" << endl << "Using default output name InclusiveSignalRegion.root" << endl;
        outFileName = "InclusiveSignalRegion.root";
    }

    TFile *outFile = new TFile(outFileName.c_str(), "RECREATE");

    //Output tree
    TTree *razorTree = new TTree("InclusiveSignalRegion", "Info on selected inclusive events");
    ...
    {
        if (!use_sys) varCollectionNames = { "" }; 
        else varCollectionNames = {"", "JESUp", "JESDown", "MESUp", "MESDown", 
    ...
    for (Long64_t jentry = 0; jentry < nentries; jentry++) 
    {
        ...
        //Initialize
        if (jentry % 10000 == 0) cout << "Processing entry " << jentry << "/" << nentries << endl;
        ...
        bool passedMonojetTrigger = false;
        ...
        vector<int> MonojetTriggerNums = helper->getMonojetTriggerNums();
        ...
        for(unsigned int itrig = 0; itrig < MonojetTriggerNums.size(); itrig++) 
        {
            if (HLTDecision[MonojetTriggerNums[itrig]]) 
            {
                passedMonojetTrigger = true;
                break;
            }
        }
            ...
            for (auto &vars : mainVars) 
            {
                ...
                { // jet does not match a lepton
                    ...
                    if (vars.first != "JESUp" && vars.first != "JESDown" && 
                            vars.first != "JERUp" && vars.first != "JERDown") 
                    { //these ones are handled below
        ...
        for (auto &vars : mainVars) 
        {
            ...
            else if ((passedHadronicTrigger || passedMonojetTrigger) && nLooseTaus + vars.second->nVetoElectrons + 
        ...
        if (!isData)
        {
            ...
            else if ((passedHadronicTrigger || passedMonojetTrigger) &&
